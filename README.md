 ...
 * gives steppy speed values like 0, 200, 400...). Alpha=0.3 gives ~10Hz
 * bandwidth which is plenty for motor control. */
#define SPEED_FILTER_ALPHA 0.3f /* 0.0=very smooth, 1.0=raw */
...
#define HEADING_KP 3.0f        /* Must correct drift from wheel asymmetry */
#define HEADING_KI 0.1f        /* Slowly correct steady-state heading error */
#define HEADING_KD 0.05f       /* Very light damping */
...
#define SPEED_FF_GAIN 1.5f /* PWM per (tick/s) */
...
void Robot_Update(void) {
  ...
  case ROBOT_STATE_IDLE:
    /* Reset filter when stopped to prevent lag on restart */
    filtered_speed_left = 0.0f;
    filtered_speed_right = 0.0f;
  ...
  case ROBOT_STATE_MOVING: {
    ...
    /* Speed setpoints: base ± heading correction */
    float sp_left = robot.target_speed + heading_correction;
    float sp_right = robot.target_speed - heading_correction;
    /* Feedforward: estimate base PWM from target speed */
    float ff_left = sp_left * SPEED_FF_GAIN;
    float ff_right = sp_right * SPEED_FF_GAIN;
    ...
    float out_left = ff_left + pid_left;
    float out_right = ff_right + pid_right;
